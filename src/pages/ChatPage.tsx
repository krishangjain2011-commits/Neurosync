...
const QUICK_PROMPTS = [
  "My child is having a meltdown. What should I do right now?",
  "Suggest a calming routine for before bedtime.",
  "How can I help with sensory overload?",
  "What are ADHD-friendly focus techniques for children?",
];
...
export default function ChatPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([{
    ...
    content: `Hello! I'm NeuroSync, your AI caregiving assistant.\n\nHow can I support you today?`,
  }]);
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      setMessages(p => p.map(m => m.id === assistId ? { ...m, content: "Sorry, I couldn't process that request." } : m));
    } finally {
      setStreaming(false);
    }
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", backgroundColor: "var(--canvas)" }}>
      ...
      <div style={{
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: "1rem", fontWeight: 700 }}>Helpful Chat</h1>
          <p style={{ margin: 0, fontSize: "0.72rem", color: "var(--text-muted)" }}>
            {activeChild?.onboarding_data?.childName
              ? `Supporting ${activeChild.onboarding_data.childName}`
              : "Real-time caregiving support"}
          </p>
        </div>
        {!isOnline && (
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "1.25rem 1.5rem" }}>
        {messages.length === 1 && (
          <div style={{ marginBottom: "1.25rem" }}>
            <p style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginBottom: "0.5rem" }}>Quick prompts:</p>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: "0.75rem", alignItems: "flex-end" }}>
          <textarea value={input} onChange={e => setInput(e.target.value)}
            ...
            placeholder={isOnline ? "Ask me anything about caregiving, behavior, routines…" : "Offline — reconnect to use chat"}
            rows={1}
        ...
        <p style={{ margin: "0.35rem 0 0", fontSize: "0.7rem", color: "var(--text-muted)" }}>Enter to send · Shift+Enter for new line</p>
      </div>
    </div>
  );
}
